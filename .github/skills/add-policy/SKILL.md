...
Policies allow enterprise administrators to lock configuration settings via OS-level mechanisms (Windows Group Policy, macOS managed preferences, Linux config files) or via VectorCode account-level policy data. This skill covers the complete procedure.
...
If this policy should also be controllable via VectorCode account policy data (from `IPolicyData`), add a `value` function:
