...
| Unassigned notes / moving artifacts | Graph document create accepts no project links; links-only revision-checked updates retain document identity | Native notes now create without a project using explicit team scope. Graph returns permission-filtered links: inbox classification and safe moves require server APIs over complete records, not client link-list replacement. Voice transcript-to-Graph association is still missing |
...
editing/recovery, assignment, search and export through explicit APIs.
[Local capture/playback/export](local-recordings.md) is also implemented through
explicit service APIs; physical-device and packaged acceptance remain open. Local project identity and service
