...
| Unassigned notes / moving artifacts | Graph document create accepts no project links; links-only revision-checked updates retain document identity | Native create currently requires a project; native parser drops non-project links, so extend it before exposing move/link controls. Voice transcript-to-Graph association is still missing |
