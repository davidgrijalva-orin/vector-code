...
export class GitFileSystemProvider implements FileSystemProvider {
	...
	private async getOrOpenRepository(uri: string | Uri): Promise<Repository | undefined> {
		...
		// In case of the empty window, no repositories are open so we need to explicitly open a
		// repository before we can serve git content for the given git resource.
