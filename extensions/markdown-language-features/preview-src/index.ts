...
// Make sure to sync Vector Code state here
...
onceDocumentLoaded(() => {
	...
	if (typeof scrollProgress === 'number' && !settings.settings.fragment) {
		doAfterImagesLoaded(() => {
			...
			// Always set scroll of at least 1 to prevent Vector Code's webview code from auto scrolling us
...
document.addEventListener('click', event => {
	...
	while (node) {
		if (node.tagName && node.tagName === 'A' && (node as HTMLAnchorElement).href) {
			...
			// If original link doesn't look like a url, delegate back to Vector Code to resolve
