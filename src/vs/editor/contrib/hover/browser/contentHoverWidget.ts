...
export class ContentHoverWidget extends ResizableContentWidget {
	...
	private _setHoverWidgetMaxDimensions(width: number | string, height: number | string): void {
		...
		this._hover.containerDomNode.style.setProperty('--vectorcode-hover-maxWidth', typeof width === 'number' ? `${width}px` : width);
		this._layoutContentWidget();
	}
	...
	private _isHoverTextOverflowing(): boolean {
		...
		this._hover.containerDomNode.style.setProperty('--vectorcode-hover-whiteSpace', 'nowrap');
		this._hover.containerDomNode.style.setProperty('--vectorcode-hover-sourceWhiteSpace', 'nowrap');

		const overflowing = Array.from(this._hover.contentsDomNode.children).some((hoverElement) => {
			return hoverElement.scrollWidth > hoverElement.clientWidth;
		});

		this._hover.containerDomNode.style.removeProperty('--vectorcode-hover-whiteSpace');
		this._hover.containerDomNode.style.removeProperty('--vectorcode-hover-sourceWhiteSpace');
