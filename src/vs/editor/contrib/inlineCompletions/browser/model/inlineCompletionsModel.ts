...
function skuFromAccount(account: IDefaultAccount | null): InlineSuggestSku | undefined {
	if (account?.entitlementsData?.access_type_sku && account?.entitlementsData?.plan) {
		return { type: account.entitlementsData.access_type_sku, plan: account.entitlementsData.plan };
	}
	return undefined;
}
