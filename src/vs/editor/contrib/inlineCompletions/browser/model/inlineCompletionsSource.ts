...
export class InlineCompletionsSource extends Disposable {
	...
	private _sendInlineCompletionsRequestTelemetry(
		...
		if (!requestResponseInfo.providers.some(p => isTelemetryForwardingExtension(p.providerId?.extensionId))) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			...forwardToChannelIf(requestResponseInfo.providers.some(p => isTelemetryForwardingExtension(p.providerId?.extensionId))),
