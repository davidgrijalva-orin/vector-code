...
export class RemoteTunnelService extends Disposable implements IRemoteTunnelService {
	...
	private getTunnelCommandLocation() {
		if (!this._tunnelCommand) {
			...
			if (isMacintosh) {
				// appRoot = /Applications/Vector Code Insiders.app/Contents/Resources/app
				// bin = /Applications/Vector Code Insiders.app/Contents/Resources/app/bin
				binParentLocation = this.environmentService.appRoot;
			} else if (isWindows) {
				if (this.productService.win32VersionedUpdate) {
					// appRoot = C:\Users\<name>\AppData\Local\Programs\Vector Code Insiders\<version>\resources\app
					// bin = C:\Users\<name>\AppData\Local\Programs\Vector Code Insiders\bin
					binParentLocation = dirname(dirname(dirname(this.environmentService.appRoot)));
				} else {
					// appRoot = C:\Users\<name>\AppData\Local\Programs\Vector Code Insiders\resources\app
					// bin = C:\Users\<name>\AppData\Local\Programs\Vector Code Insiders\bin
