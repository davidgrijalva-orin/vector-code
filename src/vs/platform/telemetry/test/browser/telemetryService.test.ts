...
class ErrorTestingSettings {
	...
	public fullBuiltinExtensionPath: string = '/Applications/Vector Code.app/Contents/Resources/app/extensions/git/out/git.js:42:1234';
...
suite('TelemetryService', () => {
	...
	test('Unexpected Error Telemetry removes PII but preserves extension path', sinonTestFn(function (this: any) {
		...
		try {
			...
			assert.strictEqual(testAppender.events[0].data.callstack.indexOf('/Applications/Vector Code.app'), -1, 'App path should be redacted from built-in extension path');
