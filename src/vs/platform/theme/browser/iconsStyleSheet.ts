	...
	return {
		...
		getCSS(): css.CssFragment {
			...
			for (const contribution of iconRegistry.getIcons()) {
				...
				const fontFamilyVar = css.inline`--vectorcode-icon-${css.className(contribution.id)}-font-family`;
				const contentVar = css.inline`--vectorcode-icon-${css.className(contribution.id)}-content`;
				if (fontContribution) {
