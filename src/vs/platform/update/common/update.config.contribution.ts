...
configurationRegistry.registerConfiguration({
	...
	properties: {
		'update.mode': {
			...
			description: localize('updateMode', "Configure whether you receive automatic updates. Requires a restart after change. The updates are fetched from the Vector Code update service."),
			...
			policy: {
				...
				minimumVersion: '1.67',
				localization: {
					description: { key: 'updateMode', value: localize('updateMode', "Configure whether you receive automatic updates. Requires a restart after change. The updates are fetched from the Vector Code update service."), },
					enumDescriptions: [
						{
							key: 'none',
		...
		'update.enableWindowsBackgroundUpdates': {
			...
			description: localize('enableWindowsBackgroundUpdates', "Enable to download and install new Vector Code versions in the background."),
		...
		'update.showReleaseNotes': {
			...
			default: true,
			scope: ConfigurationScope.APPLICATION,
			description: localize('showReleaseNotes', "Show Release Notes after an update. The Release Notes are fetched from the Vector Code update service."),
			tags: ['usesOnlineServices'],
		},
