...
suite('VectorCodeCodexPendingRequests', () => {
	...
	test('timeout rejects and ignores a late response', async () => {
		...
		await result.then(
			...
			error => strictEqual(error.message, 'Codex App Server request timed out: model/list'),
		);
	...
	test('shutdown rejects every pending request', async () => {
		const requests = new VectorCodeCodexPendingRequests();
		const first = requests.create('request-4', 'thread/read', 1_000).catch(error => error.message);
		const second = requests.create('request-5', 'plugin/list', 1_000).catch(error => error.message);

		requests.rejectAll(new Error('Codex stopped.'));
		strictEqual(await first, 'Codex stopped.');
		strictEqual(await second, 'Codex stopped.');
		strictEqual(requests.size, 0);
	});
	...
	test('rejects duplicate correlation identifiers without replacing the original', async () => {
		...
		await duplicate.then(
			...
			error => strictEqual(error.message, 'Duplicate Codex App Server request identifier: request-6'),
		);
