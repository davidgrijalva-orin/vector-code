...
suite('VectorGraph ticket contracts', () => {
	...
	test('IPC exposes explicit operations but cannot invoke private helpers', async () => {
		...
		const service: IVectorGraphService = {
			...
			listCanvases: async () => [], getCanvas: async () => { throw new Error('Unused'); }, listDocuments: async () => [], getDocument: async () => { throw new Error('Unused'); }, createDocument: async () => { throw new Error('Unused'); }, saveDocument: async () => { throw new Error('Unused'); },
