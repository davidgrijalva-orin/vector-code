...
- **Image extraction**: Explorer integration builds a **sections-based** collection from selected media files or sibling media in the selected folder.
...
- **Collection ID = media collection identity**: `collection.id` is used for stable dedup via `EditorInput.matches()`.
