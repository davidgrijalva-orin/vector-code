	...
	constructor(
		...
		this._register(autorunWithStore((reader, store) => {
			...
			const statusMap: Record<typeof status, { shortLabel: string; label: string; loading: boolean }> = {
				...
				ghostText: { shortLabel: '$(lightbulb)', label: '$(vectorcode) ' + localize('inlineCompletionAvailable', "Inline completion available"), loading: false, },
				inlineEdit: { shortLabel: '$(lightbulb-sparkle)', label: '$(vectorcode) ' + localize('inlineEditAvailable', "Inline edit available"), loading: false, },
				noSuggestion: { shortLabel: '$(circle-slash)', label: '$(vectorcode) ' + localize('noInlineSuggestionAvailable', "No inline suggestion available"), loading: false, },
			};
