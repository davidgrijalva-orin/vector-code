...
export class BaseIssueReporterService extends Disposable {
	...
	private updateInternalFilingNote(container: HTMLElement) {
		...
		filingNote.textContent = escape(localize('internalPreviewMessage', 'If your vectorcode debug logs contain private information:'));
	}
	...
	public setSourceOptions(): void {
		...
		sourceSelect.append(this.makeOption(IssueSource.VSCode, localize('vscode', "Vector Code"), false));
		sourceSelect.append(this.makeOption(IssueSource.Extension, localize('extension', "A Vector Code extension"), false));
	...
	public async renderBlocks(): Promise<void> {
		...
		if (selectedExtension && this.nonGitHubIssueUrl) {
			...
			reset(descriptionTitle, localize('handlesIssuesElsewhere', "This extension handles issues outside of Vector Code"));
