				...
				if (active) {
					this._terminateAll().then((responses) => {
						...
						if (response.code && response.code === TerminateResponseCode.ProcessNotFound) {
							this._notificationService.error(nls.localize('TerminateAction.noProcess', 'The launched process doesn\'t exist anymore. If the task spawned background tasks exiting Vector Code might result in orphaned processes.'));
						} else {
