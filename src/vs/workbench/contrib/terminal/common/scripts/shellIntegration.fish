...
# ---------------------------------------------------------------------------------------------
#
# Vector Code terminal integration for fish
#
# Manual installation:
#
...
if not set -q VSCODE_PYTHON_AUTOACTIVATE_GUARD
	...
	if test -n "$VSCODE_PYTHON_FISH_ACTIVATE"; and test "$TERM_PROGRAM" = "vscode"
		...
		if test $__vsc_activation_status -ne 0
				builtin printf '\x1b[0m\x1b[7m * \x1b[0;103m Vector Code Python fish activation failed with exit code %d \x1b[0m \n' "$__vsc_activation_status"
		end
	end
...
# Helper function
	function __vsc_esc -d "Emit escape sequences for Vector Code shell integration"
