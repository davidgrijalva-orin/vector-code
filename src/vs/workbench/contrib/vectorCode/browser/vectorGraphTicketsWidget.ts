...
export class VectorGraphTicketsWidget extends Disposable {
	...
	private connection!: HTMLDetailsElement;
	...
	render(container: HTMLElement): void {
		...
		const connection = this.connection = append(this.root, $<HTMLDetailsElement>('details.vector-graph-connection'));
		this.account = append(connection, $('summary.vector-graph-tickets__account'));
		const accountActions = append(connection, $('.vector-graph-tickets__toolbar'));
		...
		const toolbar = append(connection, $('.vector-graph-tickets__toolbar'));
		...
		const filters = append(this.root, $('.vector-graph-tickets__filters'));
		this.activeWork = append(this.root, $('.vector-graph-active-work'));
		this.scope = append(filters, $<HTMLSelectElement>('select')); this.scope.setAttribute('aria-label', localize('workTicketScope', 'Ticket scope'));
		...
		this.assignee = append(filters, $<HTMLSelectElement>('select')); this.assignee.setAttribute('aria-label', localize('workAssigneeFilter', 'Filter by assignee'));
		...
		this.search = append(filters, $<HTMLInputElement>('input.vector-graph-tickets__search'));
		...
		this.category = append(filters, $<HTMLSelectElement>('select.vector-graph-tickets__filter'));
	...
	private renderAccount(): void {
		if (!this.session.workspaces.length || this.session.authorization) { this.connection.open = true; }
	...
	private renderTickets(): void {
		...
		for (const ticket of visible) {
			...
			append(button, $('.vector-graph-tickets__identifier')).textContent = ticket.identifier;
			append(button, $('.vector-graph-tickets__title')).textContent = ticket.title;
			append(button, $('.vector-graph-tickets__meta')).textContent = ticket.status;
