...
suite('VectorCode local note file-model lifecycle', () => {
	...
	for (const additionalTab of [false, true]) {
		test('edit, save, reopen, conflict and hot-exit recovery: ' + (additionalTab ? 'additional tab' : 'legacy first tab'), async () => {
			const id = '658d2b51-5118-46d4-8b60-bf1954501284';
			const tabId = 'b2470f8e-e052-4e3a-881d-ab967858f66e';
			const resource = localNoteResource(id, additionalTab ? tabId : undefined);
			...
			const graph = {
				read: async () => ({ version: 1, projects: [], notes: [additionalTab ? { ...record, body: 'First tab is separate', additionalTabs: [{ ...record, id: tabId }] } : { ...record }] }),
